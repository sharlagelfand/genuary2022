...
specks <- strawberries %>%
  ...
  map(function(data) {
    ...
    tibble(size = sizes * 2) %>%
      mutate(n = 1) %>%
      arrange(-size) %>%
      pack_circles(x0 - 0.75 * r, x0 + 0.75 * r, y0 - 0.75 * r, y0 + 0.75 * r, n_errors = 10) %>%
      mutate(id = row_number()) %>%
      split(.$id) %>%
      map(function(data) {
        perlin_circle(data[["x0"]], data[["y0"]],
...
p <- ggplot() +
  ...
  fill = NA, color = lightpink, size = 2
  ) +
  geom_polygon(data = stems, aes(x = x + 0.5, y = y - 0.5, fill = colour, group = id), color = NA, alpha = 0.9) +
  geom_polygon(data = strawberries, aes(x = x + 2, y = y, fill = colour, group = id), fill = lightpink) +
  geom_polygon(data = strawberries, aes(x = x - 2, y = y, fill = colour, group = id), color = NA, alpha = 0.7) +
...
ggsave(here::here("17", "day_17_bigger.png"), p, width = 7, height = 7, dpi = 500, bg = "white")
